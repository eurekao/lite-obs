...
typedef struct lite_obs_media_source_api {
    ...
    void (*output_video2)(struct lite_obs_media_source_api *source_api, const uint8_t *video_data[MAX_AV_PLANES], const int line_size[MAX_AV_PLANES],
                          ...
                          uint32_t height);
    //only support RGBA BGRA format
    void (*output_video3)(struct lite_obs_media_source_api *source_api, const uint8_t *img_data, uint32_t img_width, uint32_t img_height, bool is_bgra);
